...
class AdminCarController
{
    public function index (Request $request, Response $response)
    {
        ...
        $carList = DB::query(
            "SELECT id, carTypeId, model, year, manufacturer, milleage, status, storeId, description, fuelType
            FROM cars"
             );
        ...
        return $view->render($response, 'admin/item_list.html.twig',[
            'itemTitle' => 'Car',
            'itemList' => $carList,
            'itemKeyList' =>  ['id', 'carTypeId', 'model', 'year', 'manufacturer', 'milleage', 'status', 'storeId', 'description', 'fuelType']
        ]);
    }
    ...
    {
        $fieldList = ['id', 'carTypeId', 'model', 'year', 'manufacturer', 'milleage', 'status', 'storeId', 'description'];
        $carId = $args['id'];
